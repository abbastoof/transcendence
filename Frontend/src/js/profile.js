...
function fetchAndRenderProfile(userData) {
    fetch(`https://localhost:3000/user/${userData.id}/`, {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        ...
        const htmlContent = `
            <div class="container mt-4">
                ...
                <img id="avatar" src="${data.imageUrl}" alt="User Profile Image" class="img-thumbnail mb-4" width="60" height="60">
        ...
        // Set up image upload form submit listener
        ...
        form.addEventListener('submit', (event) => {
            ...
            fetch(`https://localhost:3000/user/${userData.id}/image`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${userData.token}`
                },
    ...
    .catch(error => {
        ...
        // Handle specific errors, e.g., unauthorized access
        if (error.response && error.response.status === 401) {
            console.error('Unauthorized access detected');
            // Handle unauthorized access, such as redirecting to login page or clearing tokens
        }
    });
}
